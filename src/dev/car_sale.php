...
session_start();
require "config.php";
require_once __DIR__ . '/fonction_commande_voiture_achat.php';


?>
<?php
//appel a la fonction pour récupérér les donnéee utilisateur qui passe la commande
$recup_user = recupdonneeUtilisateurCommandeAchat($pdo);
if (!$recup_user["success"]) {

  header("Location: cnxn_news.php?message=connexion_necessaire");
  exit;
}
$donnee_user = $recup_user["user"];
$user_id = $_SESSION["user_id"];
?>

<?php
//appel a la fonction recup donnee vehicule de la commande
$recup_vehicule = recupVehiculeCommandeAchat($pdo);
if (!$recup_vehicule["success"]) {

  return;
}
$donnee_vehicule = $recup_vehicule["vehicule"];
?>
    ...
    <div class="container_descriptif">
      <div class="box_televersement_document" style="text-align : left; line-height: 2" ;>
        ...
        <form action="enreg_document.php" method="post" enctype="multipart/form-data"
          ...
          <input type="hidden" name="page_retour" value="car_sale.php">
